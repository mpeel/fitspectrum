...
for i in range(0,numres):
	for m in range(0,len(mapnumbers)):
		...
		if m == 0:
			...
			sigma_0 = 1.435
		smoothnoisemap(directory+'/wmap9', outdirectory, 'wmap_K_'+str(output_resolution[i])+'_'+str(mapnumbers[m]), 'wmap_band_iqumap_r9_9yr_K_v5.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],windowfunction=beamtf_K,sigma_0=sigma_0,nside=output_nside,usehealpixfits=True)
		if m == 0:
			sigma_0 = 1.466
		else:
			sigma_0 = 1.472
		smoothnoisemap(directory+'/wmap9', outdirectory, 'wmap_Ka_'+str(output_resolution[i])+'_'+str(mapnumbers[m]), 'wmap_band_iqumap_r9_9yr_Ka_v5.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],windowfunction=beamtf_Ka,sigma_0=sigma_0,nside=output_nside,usehealpixfits=True)
		if m == 0:
			sigma_0 = 2.188
		else:
			sigma_0 = 2.197
		smoothnoisemap(directory+'/wmap9', outdirectory, 'wmap_Q_'+str(output_resolution[i])+'_'+str(mapnumbers[m]), 'wmap_band_iqumap_r9_9yr_Q_v5.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],windowfunction=beamtf_Q,sigma_0=sigma_0,nside=output_nside,usehealpixfits=True)
		if m == 0:
			sigma_0 = 3.131
		else:
			sigma_0 = 3.141
		smoothnoisemap(directory+'/wmap9', outdirectory, 'wmap_V_'+str(output_resolution[i])+'_'+str(mapnumbers[m]), 'wmap_band_iqumap_r9_9yr_V_v5.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],windowfunction=beamtf_V,sigma_0=sigma_0,nside=output_nside,usehealpixfits=True)
		if m == 0:
			sigma_0 = 6.544
		else:
			sigma_0 = 6.560
		smoothnoisemap(directory+'/wmap9', outdirectory, 'wmap_W_'+str(output_resolution[i])+'_'+str(mapnumbers[m]), 'wmap_band_iqumap_r9_9yr_W_v5.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],windowfunction=beamtf_W,sigma_0=sigma_0,nside=output_nside,usehealpixfits=True)

mapnumbers = [4,5,6,7,8,9]

for i in range(0,numres):
	for j in range(0,len(mapnumbers)):
		resolution = "%.2f" % output_resolution[i]

		smoothnoisemap(directory+'planck2018/', outdirectory, resolution+'smoothed_PlanckR3fullbeam_28.4_1024_2018_mKCMBunits'+'_'+str(mapnumbers[m]), 'LFI_SkyMap_030-BPassCorrected_1024_R3.00_full.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],nside=output_nside,windowfunction=beamtf_p30,rescale=1000.0,usehealpixfits=True)
		smoothnoisemap(directory+'planck2018/', outdirectory, resolution+'smoothed_PlanckR3fullbeam_44.1_1024_2018_mKCMBunits'+'_'+str(mapnumbers[m]), 'LFI_SkyMap_044-BPassCorrected_1024_R3.00_full.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],nside=output_nside,windowfunction=beamtf_p44,rescale=1000.0,usehealpixfits=True)
		smoothnoisemap(directory+'planck2018/', outdirectory, resolution+'smoothed_PlanckR3fullbeam_70.4_1024_2018_mKCMBunits'+'_'+str(mapnumbers[m]), 'LFI_SkyMap_070-BPassCorrected_1024_R3.00_full.fits',mapnumber=mapnumbers[m],numrealisations=numrealisations,fwhm=output_resolution[i],nside=output_nside,windowfunction=beamtf_p70,rescale=1000.0,usehealpixfits=True)
